...
Game BattleshipGame;
D2DRenderer *pRenderer;

BVPoint mousePos = BVPoint(0, 0); // Position used to demonstrate clicking.

short mouseDownPrev = 1;
short mouseDownCurr = 1;
                     ...
                     _In_ int       nCmdShow)
{
	...
	while (true)
	{
		mouseDownPrev = mouseDownCurr;
		mouseDownCurr = GetAsyncKeyState(VK_LBUTTON);
		// Main message loop:
		...
		if (BattleshipGame.currentPhase == Playphase::eP1Placement || BattleshipGame.currentPhase == Playphase::eP2Placement)
		{
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow, HWND &hWnd)
{
   ...
   HRESULT hr = pRenderer->CreateGraphics(hWnd);
   if (FAILED(hr))
   {
	   return FALSE;
   }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		break;
	case WM_ERASEBKGND:
		// Do nothing to stop erasing the background before a WM_PAINT.
		break;
	case WM_DESTROY:
		PostQuitMessage(0);
		break;
...
{
	BattleshipGame.Render(hWnd);
}

void OnLButtonDown(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	if (mouseDownCurr && !mouseDownPrev)
	{
		...
		mousePos.x = mousePos.x * (pRenderer->pRT->GetSize().width / rc.right);
		mousePos.y = mousePos.y * (pRenderer->pRT->GetSize().height / rc.bottom);

		BattleshipGame.Click(mousePos);
	}
	InvalidateRect(hWnd, NULL, true);
}